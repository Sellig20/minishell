...
/*   Updated: 2022/11/12 19:00:59 by jecolmou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_write_exp(t_list **exp, t_list *cmdredir, t_data *x)
{
	...
	while (tmp)
	{
		...
		if (content->token == TOK_ENV)
		{
			...
			write_exp_one(before_eq, after_eq, cmdredir, x);
			free(before_eq);
			free(after_eq);
		}
		else
			write_exp_two(content->word, cmdredir, x);
		tmp = tmp->next;
	}
}
...
void	ft_put_export(t_list **cpenv, t_list *cmdredir, t_data *x)
{
	t_list	*tmp;
	t_words	*content;
	t_list	**exp;
...
int	ft_export(t_list *cmdredir, t_list **cpenv, t_data *x)
{
	...
	(void)x;
	if (!(cmd->next))
		return (ft_put_export(cpenv, cmdredir, x), err);
	if (!(((t_words *)cmd->next->content)->word))
		return (ft_export_error(((t_words *)cmd->content)->word), 1);
	cmd = cmd->next;
	while (cmd)
	{
